...
_VIZ: Optional[VizSliderAgent] = None

def configure_viz_slider_tool():
    global _VIZ
    _VIZ = VizSliderAgent()

class VizSliderArgs(BaseModel):
    ...
    cmaps: Optional[List[str]] = Field(
        default=None, description="Matplotlib colormap names for masks; cycles if fewer than masks."
    )
    window_center: Optional[float] = Field(default=None, description="WL center for windowing.")
    window_width: Optional[float] = Field(default=None, description="WL width for windowing.")
    per_slice_minmax: Optional[bool] = Field(
        default=None,
        description="If True, normalize each slice by its own min/max. Defaults True if no WL/WW."
    )
    interactive: bool = Field(default=True, description="True => slider across slices; False => static mid-slice.")
    max_dim: int = Field(default=512, description="Downsample longest side to <= max_dim for speed.")
...
async def viz_slider_runner(
    image_path: Optional[str] = None,
    image_array: Optional[Any] = None,
    mask_paths: Optional[List[str]] = None,
    mask_arrays: Optional[List[Any]] = None,
    alpha: float = 0.4,
    cmaps: Optional[List[str]] = None,
    window_center: Optional[float] = None,
    window_width: Optional[float] = None,
    per_slice_minmax: Optional[bool] = None,
    interactive: bool = True,
    max_dim: int = 512,
):
    if _VIZ is None:
        raise RuntimeError("VizSlider tool not configured. Call configure_viz_slider_tool() first.")
    ...
    kwargs = {
        ...
        "per_slice_minmax": per_slice_minmax,
        "interactive": interactive,
        "max_dim": max_dim,
    }
    task = Task(user_msg="Visualize CT with slice slider", files=[], kwargs=kwargs)
    return await _VIZ.run(task, _cs())
